...
{
public:
    StreamSource(Block header, std::weak_ptr<Subscriber> subscriber_, UInt64 cursor) 
        : ISource(std::move(header)), subscriber(subscriber_), client_cursor(cursor) {}
...
protected:
    ...
    Block tryReadImpl()
    {
        ...
        if (!blocks)
        {
            LOG_FATAL(&Poco::Logger::root(), "new_block {}", "!block");
            auto subs = subscriber.lock();
            std::lock_guard lock(subs->mutex);
            blocks = subs->blocks;
            it = blocks->begin();
