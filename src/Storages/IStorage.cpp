    ...
    size_t num_streams)
{
    ...
    if (select.is_stream) {
        ...
        if (select.is_cursor) {
            (*subscriber).addCursor(select.is_cursor, "");
        }
        std::weak_ptr<Subscriber> subs_ptr = subscriber;
        addNewSubscriber(subscriber);
        pipes.emplace_back(std::make_shared<StreamSource>(storage_snapshot->getSampleBlockForColumns(column_names), subs_ptr, (*subscriber).cursor.GetValue()));
    }    
